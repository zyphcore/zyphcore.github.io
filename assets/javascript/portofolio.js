...
$(document).ready(() => {
  ...
  if (mobileAndTabletCheck()) {
    $('#background').replaceWith('<div id="background" style="background-image: url(https://eskipaper.com/images/black-screen-2.jpg);"></div>');
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[zyph]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here ;)`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>zyph has a big penis $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
